...
| 15 | **v3 = evolve, not rebuild.** Keep strong existing components (FeatureCard alternating services, ProjectCard, ContactForm, layouts, OKLCH tokens) and **restyle** them to the chosen direction. Mocks are direction-finding only — NOT component replacements | 2026-06-23 | Current site already has polished, proven sections (e.g. the services FeatureCard); reuse + re-skin lowers risk and preserves quality | Must map the new design language onto existing component APIs in 3b |
